...
public class SpinningMenu extends SpinningMenuSpinner implements GestureDetector.OnGestureListener {
     ...
     */
    private static final String TAG = SpinningMenu.class.getSimpleName();
    ...
    public SpinningMenu(Context context)  {
        this(context, null);
    }

    public SpinningMenu(Context context, AttributeSet attrs)  {
        this(context, attrs, 0);
    }

    public SpinningMenu(Context context, AttributeSet attrs, int defStyle) {
        ...
        // Retrieve settings
        TypedArray arr = getContext().obtainStyledAttributes(attrs, R.styleable.SpinningMenu);
        mAnimationDuration = arr.getInteger(R.styleable.SpinningMenu_android_animationDuration, 400);
        mUseReflection = arr.getBoolean(R.styleable.SpinningMenu_UseReflection, false);	
        int selectedItem = arr.getInteger(R.styleable.SpinningMenu_SelectedItem, 0);
        int imageArrayID = arr.getResourceId(R.styleable.SpinningMenu_Items, -1);		
        TypedArray images = getResources().obtainTypedArray(imageArrayID);

        // Retrieve names
        int namesForItems = arr.getResourceId(R.styleable.SpinningMenu_Names, -1);
        ...
        int min = arr.getInteger(R.styleable.SpinningMenu_minQuantity, MIN_QUANTITY);
        int max = arr.getInteger(R.styleable.SpinningMenu_maxQuantity, MAX_QUANTITY);

        float mTheta = arr.getFloat(R.styleable.SpinningMenu_maxTheta, MAX_THETA);
    ...
    protected int getChildDrawingOrder(int childCount, int i) {
        ...
        // Sort SpinningMenu items by z coordinate in reverse order
        ArrayList<SpinningMenuItem> sl = new ArrayList<SpinningMenuItem>();
        for(int j = 0; j < childCount; j++)
        {
            SpinningMenuItem view = (SpinningMenuItem)getAdapter().getView(j,null, null);
            if(i == 0) {
                view.setDrawn(false);
            }
            sl.add((SpinningMenuItem)getAdapter().getView(j,null, null));
        }
    ...
    protected boolean getChildStaticTransformation(View child, Transformation transformation) {
        ...
        mCamera.translate(((SpinningMenuItem)child).getItemX(), ((SpinningMenuItem)child).getItemY(), 
                ((SpinningMenuItem)child).getItemZ());
        ...
        mm.setValues(values);
        ((SpinningMenuItem)child).setCIMatrix(mm);
    ...
    // Rotation class for the SpinningMenu    
    ...
    private class FlingRotateRunnable implements Runnable {
        ...
        @Override
        public void run() {
            if (SpinningMenu.this.getChildCount() == 0) {
                endFling(true);
                return;
            }			
    ...
    // Image adapter class for the SpinningMenu

    private class ImageAdapter extends BaseAdapter {

        private Context mContext;
        private SpinningMenuItem[] mImages;		
        ...
        public void SetImages(TypedArray array, TypedArray names, boolean reflected){
            ...
            mImages = new SpinningMenuItem[array.length()];
            ...
            for(int i = 0; i< array.length(); i++)
            {
                ...
                SpinningMenuItem item = new SpinningMenuItem(mContext);
                item.setIndex(i);
    ...
    private void Calculate3DPosition(SpinningMenuItem child, int diameter, float angleOffset){

        angleOffset = angleOffset * (float)(Math.PI/180.0f);    	

        float x = (- ((diameter/2)  * android.util.FloatMath.sin(angleOffset)) + (diameter/2)) - (child.getWidth()/2);
        float z = (diameter/2) * (1.0f - android.util.FloatMath.cos(angleOffset));
        float y = (- getHeight()/2) + z * android.util.FloatMath.sin((float) (Math.PI/4)) + 154;
    ...
    private int getCenterOfGallery() {
        return ((getWidth() - SpinningMenu.this.getPaddingLeft() - SpinningMenu.this.getPaddingRight()) / 2) + 
                SpinningMenu.this.getPaddingLeft();
    }	
    ...
    float getLimitedMotionScrollAmount(boolean motionToLeft, float deltaX) {
        int extremeItemPosition = motionToLeft ? SpinningMenu.this.getCount() - 1 : 0;
        View extremeChild = getChildAt(extremeItemPosition - SpinningMenu.this.getFirstVisiblePosition());
    ...
    private void makeAndAddView(int position, float angleOffset) {
        SpinningMenuItem child;

        if (!mDataChanged) {
            child = (SpinningMenuItem)mRecycler.get(position);
            if (child != null) {
            ...
            else
            {
                ...
                child = (SpinningMenuItem)mAdapter.getView(position, null, this);
    ...
    private void scrollIntoSlots(){
        ...
        ArrayList<SpinningMenuItem> arr = new ArrayList<SpinningMenuItem>();

        for(int i = 0; i < getAdapter().getCount(); i++) {
            arr.add(((SpinningMenuItem)getAdapter().getView(i, null, null)));
        }

        Collections.sort(arr, new Comparator<SpinningMenuItem>(){

            @Override
            public int compare(SpinningMenuItem c1, SpinningMenuItem c2) {
     ...
     */
    private void setUpChild(SpinningMenuItem child, int index, float angleOffset) {
    ...
    void trackMotionScroll(float deltaAngle) {
        ...
        for(int i = 0; i < getAdapter().getCount(); i++){
            ...
            SpinningMenuItem child = (SpinningMenuItem)getAdapter().getView(i, null, null);
